...
import AddSubSubCategory from "./AddSubCategory";
...
const SubSubCategories = () => {
  ...
  const fetchSizes = () => {
    axios
      .get(`${Base_url}/subSubCategory/get?page=${currentPage}&limit=${limit}&search=${search}`)
      .then((res) => {
  ...
  const removeFunction = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${Base_url}/subSubCategory/delete/${id}`)
          .then((res) => {
            if (res.status === 200) {
  ...
  return (
    <>
      <div className="flex justify-between items-center">
        <h2 className="main_title">Sub Sub Categories</h2>
      ...
      <AddSubSubCategory isModalOpen={isUpdateOpen}
