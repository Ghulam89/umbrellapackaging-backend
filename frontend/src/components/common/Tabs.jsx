    ...
    <div>
      <div className="flex gap-3 overflow-x-auto whitespace-nowrap">
        {tabs.map((tab) => (
          <button
            key={tab.title}
            className={`px-4 cursor-pointer transition-colors duration-300 ease-in-out rounded-lg text-sm py-4.5 ${
              activeTab === tab.title
      ...
      <div className="mt-4">
        {tabs.map((tab) => (
          <div
            ...
            style={{
              // Use CSS containment to isolate layout calculations and prevent forced reflows
              contain: activeTab === tab.title ? "layout style paint" : "none",
            }}
          >
            {tab.content}
          </div>
