...
export const createBanner = catchAsyncError(async (req, res, next) => {
    try {
        ...
        if (req.files?.image) {
            const filePath = path.join(__dirname, 'public/images', req.files.image[0].filename);
...
export const updateBanner = catchAsyncError(async (req, res, next) => {
    ...
    try {
        ...
        if (req.files?.image) {
            ...
            const newImagePath = `${process.env.BASEURL}/public/images/${req.files.image[0].filename}`.replace(/\\/g, '/');
