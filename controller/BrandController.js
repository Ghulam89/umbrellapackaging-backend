...
export const createBrand = catchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    const bannerPath = `${process.env.BASEURL}/public/${req.files.bannerImage[0].filename}`.replace(/\\/g, '/');
...
export const updateBrand = catchAsyncError(async (req, res, next) => {
  ...
  try {
    if (req.files?.image) {
      updateData.image = `${process.env.BASEURL}/public/${req.files.image[0].filename}`.replace(/\\/g, '/');
    }

    if (req.files?.bannerImage) {
      updateData.bannerImage = `${process.env.BASEURL}/public/${req.files.bannerImage[0].filename}`.replace(/\\/g, '/');
    }
    ...
    if (req.files?.image) {
      fs.unlinkSync(path.join(__dirname, '..', 'public', 'public', req.files.image[0].filename));
    }
    if (req.files?.bannerImage) {
      fs.unlinkSync(path.join(__dirname, '..', 'public', 'public', req.files.bannerImage[0].filename));
    }
    return next(error);
  }
});
