...
export const createCategory = catchAsyncError(async (req, res, next) => {
  ...
  try {
    const categoryData = {
      ...
      videoDescription,
      brandId,
      icon: `${process.env.BASEURL}/public/images/${req.files.icon[0].filename}`.replace(/\\/g, '/'),
      image: `${process.env.BASEURL}/public/images/${req.files.image[0].filename}`.replace(/\\/g, '/'),
      bannerTitleFirst,
      bannerContentFirst,
      bannerImageFirst: `${process.env.BASEURL}/public/images/${req.files.bannerImageFirst[0].filename}`.replace(/\\/g, '/'),
      bannerTitleSecond,
      bannerContentSecond,
      bannerImageSecond: `${process.env.BASEURL}/public/images/${req.files.bannerImageSecond[0].filename}`.replace(/\\/g, '/'),
      bannerTitleThird,
      bannerContentThird,
      bannerImageThird: `${process.env.BASEURL}/public/images/${req.files.bannerImageThird[0].filename}`.replace(/\\/g, '/'),
      bannerTitleFourth,
      bannerContentFourth,
      bannerImageFourth: `${process.env.BASEURL}/public/images/${req.files.bannerImageFourth[0].filename}`.replace(/\\/g, '/'),
    };
  ...
  try {
    if (req.files?.image) {
      ...
      if (existingCategory.image) {
        ...
        const fullOldPath = path.join(__dirname, 'public/images', oldImagePath);
    ...
    if (req.files?.icon) {
      const iconPath = `${process.env.BASEURL}/public/images/${req.files.icon[0].filename}`.replace(/\\/g, '/');
      ...
      if (existingCategory.icon) {
        ...
        const fullOldPath = path.join(__dirname, 'public/images', oldIconPath);
        if (fs.existsSync(fullOldPath)) {
          fs.unlinkSync(fullOldPath);
        }
      }
    ...
    for (const field of bannerFields) {
      if (req.files?.[field]) {
        const filePath = `${process.env.BASEURL}/public/images/${req.files[field][0].filename}`.replace(/\\/g, '/');
        ...
        if (existingCategory[field]) {
          ...
          const fullOldPath = path.join(__dirname, 'public/images', oldPath);
          if (fs.existsSync(fullOldPath)) {
            fs.unlinkSync(fullOldPath);
          }
        }
