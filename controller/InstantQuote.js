...
export const createInstantQuote = catchAsyncError(async (req, res, next) => {
  ...
  res.status(200).json({
    status: "success",
    message: "Instant Quote create successfully!",
...
export const updateInstantQuote = catchAsyncError(async (req, res, next) => {
    ...
    if (!existingInstantQuote) {
      return res.status(404).json({ message: "Instant quote not found" });
    }
    ...
    res.status(200).json({
      ...
      message: "Instant Quote updated successfully!",
    });
  });
...
export const deleteInstantQuoteById = async (req, res, next) => {
  ...
  try {
    ...
    res.json({
      status: "success",
      message: "Instant Quote deleted successfully!",
    });
  } catch (error) {
    console.log(error);
    next(error);
